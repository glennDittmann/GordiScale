...
byte read_LCD_buttons()
{
  ...
  // note that we mapped the boundaries to match our lcd
  if (adc_key_in > 1000) return btnNONE;
  if (adc_key_in < 60)   return btnRIGHT;  // original value: 50
  if (adc_key_in < 200)  return btnUP;  // original value: 250
  if (adc_key_in < 400)  return btnDOWN;  // original value: 450
  if (adc_key_in < 600)  return btnLEFT;  // original value: 650
  if (adc_key_in < 800)  return btnSELECT;  // original value: 850
